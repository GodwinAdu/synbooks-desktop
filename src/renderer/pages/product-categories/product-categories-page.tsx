 ...
 * CRUD using dedicated product_categories table (syncs with cloud).
 */
...
export function ProductCategoriesPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res: any = await api.get("/categories/product");
  ...
  const handleAdd = async () => {
    ...
    try {
      await api.post("/categories/product", { name: newName.trim() });
      setNewName("");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/categories/product/${id}`);
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.put(`/categories/product/${editingId}`, { name: editName.trim() });
      setEditingId(null);
  ...
  return (
    <div className="p-6 space-y-6">
      <Heading title="Product Categories" description="Organize products by category" />
      <Separator />

      <div className="max-w-2xl space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Tag className="h-5 w-5 text-emerald-600" /> Add Category</CardTitle>
          </CardHeader>
          <CardContent>
