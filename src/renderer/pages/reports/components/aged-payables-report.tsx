...
export function AgedPayablesReport() {
  ...
  const handleExportCSV = () => {
    ...
    const rows: string[][] = [
      ...
      [],
      ["Bucket", "Vendor", "Bill #", "Due Date", "Total", "Outstanding", "Days Overdue"],
    ];
    const addBucket = (label: string, items: AgingEntry[]) => {
      items.forEach((e) => rows.push([label, e.vendorName || "", e.billNumber || "", e.dueDate, e.totalAmount.toFixed(2), e.outstanding.toFixed(2), e.daysOverdue.toString()]));
    };
    ...
    a.href = url;
    a.download = `aged-payables-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="p-6 space-y-6">
      <ReportHeader
        title="Aged Payables"
        subtitle="Outstanding vendor bills by aging bucket"
        showDateRange={false}
                      ...
                      <>
                        ...
                        {items.map((entry) => (
                          <tr key={entry.id} className="border-b hover:bg-muted/30 transition-colors">
                            <td className="py-2.5 px-4">{entry.vendorName}</td>
                            <td className="py-2.5 px-4 font-mono text-xs">{entry.billNumber || "—"}</td>
